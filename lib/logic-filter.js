...
LogicFilter.prototype.addBurster = function (burster) {
  if(this.bursters === null || this.bursters === undefined) {
    this.bursters = [];
  }
  this.bursters.push(burster);
};
...
/**
 * 根据compiled的criteria对数据进行分析,分析器由resolver提供
 * @param resolver
 */
...
LogicFilter.prototype.interpret = function (remain, model) {
  ...
  while(remain !== null && remain !== undefined && remain.length > 0) {
    var operator;
...
LogicFilter.prototype.handleArgument = function (input, model) {
  ...
  if((result = regexp.exec(input)) !== null) {
    ...
    this.bursters.forEach(burster => {
      // burst it into new model if true
      if(burster.filter(node)) {
        console.log('Bursting...', node);

        var subCriteria = burster.resolver(node);
        var subModel = [];
        _this.interpret(subCriteria, subModel);

        // 如果无法interpret, 返回的model是一个空数组, 忽略这个节点的解释并返回原值
        if(subModel[0] !== undefined) {
          node = subModel[0];
        }
      }
    });
